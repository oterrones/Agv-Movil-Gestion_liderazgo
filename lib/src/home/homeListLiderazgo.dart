...
class _homeListLiderazgoState extends State<HomeListLiderazgo> {
  ...
  void _refreshJournals() async {
    ...
    final data_tamanio_lista = await DBProvider.db.getAllResultadosPacking_validar_existencia();
    setState(() {
      _journals = data;
  ...
  Future<void> _sincronizacion_data() async {
    ...
    final data = await DBProvider.db.enviarDataLiderazgoPackingAWS();
    setState(() {
...
class _SelectCapacitacionState extends State<SelectCapacitacion> {
  ...
  Widget build(BuildContext context) {
    ...
    msn_accion = (widget.tipo_accion == 'R' ? 'Registrar' : 'Actualizar');
  ...
  Future<bool> showExitPopup() async {
    return await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Ingrese la evaluación de satisfacción "),
        actions: [
          ElevatedButton(
            onPressed: () => Navigator.of(context).pop(false),
            //return false when click on "NO"
